...
pub async fn erase_part(app: &AppHandle, part: &str, xml_content: &str, config: &Config) {
    ...
    #[cfg(target_os = "windows")] {
        ...
        command_util::exec_cmd_with_msg(&format!("Erase partition {}", part), &app, &config, &cmd).await;
    }
    #[cfg(target_os = "linux")] {
...
pub async fn read_part(app: &AppHandle, part: &str, xml_content: &str, folder: &str, config: &Config) {
    ...
    #[cfg(target_os = "windows")] {
        ...
        command_util::exec_cmd_with_msg(&format!("Read partition {}...", part), &app, &config, &cmd).await;
    }
    #[cfg(target_os = "linux")] {
        let cmd = [&config.fh_loader_path_linux, &config.fh_port_conn_str_linux, "--memoryname=ufs", "--convertprogram2read",
...
pub async fn reboot_to(app: &AppHandle, msg: &str, xml: &str, config: &Config) {
    ...
    #[cfg(target_os = "windows")] {
        ...
        command_util::exec_cmd_with_msg("Writ misc partition", &app, &config, &cmd).await;
        // send reboot command
        ...
        command_util::exec_cmd_with_msg(&msg, &app, &config, &cmd).await;
    }
    #[cfg(target_os = "linux")] {
...
pub async fn reboot_to_edl(app: &AppHandle, config: &Config) {
    ...
    #[cfg(target_os = "windows")] {
        let cmd = ["cmd", "/c", &config.fh_loader_path, &config.fh_port_conn_str, "--memoryname=ufs", 
            "--sendxml=res/cmd.xml", "--noprompt", "--skip_configure", "--mainoutputdir=res"];
        command_util::exec_cmd_with_msg("Reboot to EDL", &app, &config, &cmd).await;
    }
    #[cfg(target_os = "linux")] {
...
pub async fn write_part(app: &AppHandle, part: &str, xml_content: &str, dir_path: &str, config: &Config) {
    ...
    #[cfg(target_os = "windows")] {
        let cmd = ["cmd", "/c", &config.fh_loader_path, &config.fh_port_conn_str, 
            ...
            "--noprompt", "--skip_configure", "--mainoutputdir=res"];
        command_util::exec_cmd_with_msg(&format!("Writ partition {}", part), &app, &config, &cmd).await;
    }
    #[cfg(target_os = "linux")] {
